...
ydk::path::Repository::create_root_schema(const std::vector<path::Capability> & capabilities)
{
    ...
    for (auto c : capabilities)
    {
        for (auto d: c.deviations)
        {
            ...
            if (!res) {
                YLOG_ERROR("Unable to parse deviation module: {}. This model cannot be used with YDK.", d);
                continue;
            }
        }
        ...
        if (!p) {
            YLOG_ERROR("Unable to parse module: {}. This model cannot be used with YDK", c.module);
            continue;
        }
