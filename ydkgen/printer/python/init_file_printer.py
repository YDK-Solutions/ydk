...
""")
    ...
    def print_nmsp_augment_finder_init(self, _, is_meta=False):
        """Print init file to patch generated APIs."""
        relative_dot = '.' if is_meta else ''
        meta_path = '._meta' if is_meta else ''

        self.ctx.str("""\"\"\"Craft and import module at  runtime.\"\"\"
...
def _get_augments_bundles():
    ...
    for (_, name, ispkg) in pkgutil.iter_modules(models.__path__):
        if ispkg:
            try:
                aug_mod = importlib.import_module('ydk.models.%s._aug{1}' % name)
